...
int baronEffect(int choice1, struct gameState *state)
{
    ...
    {                               //Boolean true or going to discard an estate
        int hand_iterator = 0;      //Iterator for hand!
        int card_not_discarded = 1; //Flag for discard set!
        while (card_not_discarded)
        {
            if (state->hand[currentPlayer][hand_iterator] == estate)
            {                      //Found an estate card!
                state->coins += 4; //Add 4 coins to the amount of coins
                state->discard[currentPlayer][state->discardCount[currentPlayer]] = state->hand[currentPlayer][hand_iterator];
                state->discardCount[currentPlayer]++;
                for (; hand_iterator < state->handCount[currentPlayer]; hand_iterator++)
                {
                    state->hand[currentPlayer][hand_iterator] = state->hand[currentPlayer][hand_iterator + 1];
                }
                state->hand[currentPlayer][state->handCount[currentPlayer]] = -1;
                state->handCount[currentPlayer]--;
                card_not_discarded = 0; //Exit the loop
            }
            else if (hand_iterator > state->handCount[currentPlayer])
            {
                if (DEBUG)
                {
...
int minionEffect(int choice1, int choice2, struct gameState *state, int handPos)
{
    ...
    {
        //BUG
        state->coins = state->coins + 3; //Should be 2
    }
